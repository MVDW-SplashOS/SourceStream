...
def push(packages):
    if SourceStream.YAML_CONFIG["push"]["type"] == "sftp":
        ...
        logger.log.fail("Invalid push type '" + SourceStream.YAML_CONFIG["push"]["type"] + "' in configuration.")
        exit(1)
    ...
    for package in packages:
        ...
        with paramiko.SSHClient() as ssh:
            try:
                ...
                sftp = ssh.open_sftp()
                logger.log.info("Connected to '" + sftp_host + ":" + sftp_port + "'");
                ...
                logger.log.fail("Failed to connecto to '" + sftp_host + ":" + sftp_port + "'")
                exit(1)
            except paramiko.ssh_exception.SSHException:
                logger.log.fail("An unknown error occurred while connecting")
                exit(1)
            except FileNotFoundError:
                logger.log.fail("The path '" + sftp_path + "' does not exist.")
